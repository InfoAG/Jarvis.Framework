...
AbstractExpression::ExpressionP GreaterOrEqual::executeExpression(Scope &scope, const LoadFunc &load, const PrintFunc &print, ExecOption execOption) const
{
    ...
    else return make_unique<GreaterOrEqual>(std::move(firstOpResult), std::move(secondOpResult));
}

TypeInfo GreaterOrEqual::typeCheck(const TypeCollection &candidates, Scope &scope)
{
...
bool GreaterOrEqual::equals(const AbstractStatement *other) const
{
    return typeid(*other) == typeid(GreaterOrEqual) && first_op->equals(static_cast<const GreaterOrEqual*>(other)->getFirstOp().get()) && second_op->equals(static_cast<const GreaterOrEqual*>(other)->getFirstOp().get());
}
