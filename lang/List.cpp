...
AbstractExpression::ExpressionP List::executeExpression(Scope &scope, const LoadFunc &load, const PrintFunc &print, ExecOption execOption) const
{
    Expressions result;
    for (const auto &op : operands) result.emplace_back(op->executeExpression(scope, load, print, execOption));
...
bool List::equals(const AbstractStatement *other) const
{
...
List List::operator/(const List &other) const
{
    ...
    for (const auto &op : operands) {
        result.emplace_back(make_unique<Division>(op->copyEx(), (*itOther)->copyEx()));
        ++itOther;
    }
...
List List::operator/(const NumberValue &other) const
{
    Expressions result;
    for (const auto &op : operands)
        result.emplace_back(make_unique<Division>(op->copyEx(), ExpressionP(static_cast<AbstractExpression*>(other.copy().get()))));
    return List(std::move(result));
}
