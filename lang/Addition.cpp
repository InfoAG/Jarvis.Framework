...
double &Addition::accessMonomValue(MonomValues &values, Expressions monom) const
{
...
AbstractExpression::ExpressionP Addition::executeExpression(Scope &scope, const LoadFunc &load, const PrintFunc &print, ExecOption execOption) const
{
    ...
    std::map<std::vector<unsigned int>, Expressions> matrixByDimensions;
    for (auto &operand : mergedOperands) {
        if (typeid(*operand) == typeid(NumberValue))
            ...
            if (vec.getX() == nullptr) vec = std::move(*static_cast<Vector*>(operand.get()));
            else vec = Vector(Addition(std::move(static_cast<Vector*>(operand.get())->getX()), std::move(vec.getX())).executeExpression(scope, load, print, execOption), Addition(std::move(static_cast<Vector*>(operand.get())->getY()), std::move(vec.getY())).executeExpression(scope, load, print, execOption), Addition(std::move(static_cast<Vector*>(operand.get())->getZ()), std::move(vec.getZ())).executeExpression(scope, load, print, execOption));
    ...
    if (numberValue != 0) {
        if (vec.getX() != nullptr) {
            vec = Vector(Addition(make_unique<NumberValue>(numberValue), std::move(vec.getX())).executeExpression(scope, load, print, execOption), Addition(make_unique<NumberValue>(numberValue), std::move(vec.getY())).executeExpression(scope, load, print, execOption), Addition(make_unique<NumberValue>(numberValue), std::move(vec.getZ())).executeExpression(scope, load, print, execOption));
    ...
    if (vec.getX() != nullptr) mergedOperands.emplace_back(make_unique<Vector>(std::move(vec)));
    for (auto &list : listByDimension) mergedOperands.emplace_back(List(std::move(list.second)).executeExpression(scope, load, print, execOption));
    for (auto &item : monomValues) { 
        if (item.second != 0) {
            Expressions workaroundVec;
...
bool Addition::equals(const AbstractStatement *other) const
{
